...
class CombineContentKtTripleTest {
    ...
    fun setup() {
        ...
        testCombinedABC = combineContent(aSubject, bSubject, cSubject)
            .mapData { (a, b, c) -> a + b + c }
            .extractContent()
    }
    ...
    fun `Combined values when both a and b has values`() = runFlowTest(testCombinedABC) {
        ...
        Assertions.assertEquals("ABC", expectMostRecentItem())
        expectNoEvents()
    }
    ...
    fun `Latest value from a is combined when b emits`() = runFlowTest(testCombinedABC) {
        ...
        Assertions.assertEquals("BBC", expectMostRecentItem())
        expectNoEvents()
    }
    ...
    fun `Latest value from b is combined when a emits`() = runFlowTest(testCombinedABC) {
        ...
        Assertions.assertEquals("ACC", expectMostRecentItem())
        expectNoEvents()
    }
    ...
    fun `Latest value from c is combined when a emits`() = runFlowTest(testCombinedABC) {
        ...
        Assertions.assertEquals("ABD", expectMostRecentItem())
        expectNoEvents()
    }
}