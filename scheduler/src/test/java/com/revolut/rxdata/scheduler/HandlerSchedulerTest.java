...
import com.revolut.rxdata.testutil.CountingRunnable;
...
public final class HandlerSchedulerTest {
    ...
    public HandlerSchedulerTest(boolean async) {
        this.scheduler = new ImmediateHandlerScheduler(new Handler(Looper.getMainLooper()), async);
        this.async = async;
    }
    ...
    public void workerScheduleOnceDisposedDoesNotRun() {
        ...
        CountingRunnable counter = new CountingRunnable();
        Disposable disposable = worker.schedule(counter, 1, MILLISECONDS);
        disposable.dispose();
    ...
    public void workerUnsubscriptionDuringSchedulingCancelsScheduledAction() {
        ...
        CountingRunnable counter = new CountingRunnable();
        worker.schedule(counter, 1, MILLISECONDS);
